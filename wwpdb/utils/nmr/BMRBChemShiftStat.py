...
class BMRBChemShiftStat:
    ...
    def __init__(self, verbose: bool = False, log: IO = sys.stderr, ccU: Optional[ChemCompUtil] = None) -> None:
        ...
        # directory
        self.__work_dir = os.path.join(os.path.dirname(__file__), 'bmrb_cs_stat')
    ...
    def get(self, comp_id: str, diamagnetic: bool = True
            ) -> List[dict]:
    ...
    def __get(self, comp_id: str, diamagnetic: bool = True
              ) -> List[dict]:
    ...
    def getGeminalAtom(self, comp_id: str, atom_id: str
                       ) -> Optional[str]:
        ...
        try:
            ...
            is_proton = atom_id[0] in PROTON_BEGIN_CODE
            ...
            if 'geminal' in d:
                ...
                return next(item['atom_id'] for item in cs_stat
                            if item['desc'] == d and item['atom_id'] != atom_id and item['atom_id'] == f"{atom_id}'")
    ...
    @functools.lru_cache(maxsize=128)
    def getAromaticAtoms(self, comp_id: str, excl_minor_atom: bool = False, primary: bool = False
                         ) -> List[str]:
    ...
    def getMethylAtoms(self, comp_id: str
                       ) -> List[str]:
    ...
    def getCentroidAtoms(self, comp_id: str, excl_minor_atom: bool = False, polypeptide_like: bool = False,
        ...
        if comp_id in self.__std_comp_ids or polypeptide_like:
            return [item['atom_id'] for item in cs_stat
                    if item['atom_id'] not in bb_atoms
                    and item['atom_id'][0] not in PROTON_BEGIN_CODE
                    and (not excl_minor_atom or (excl_minor_atom and item['primary']))]
    ...
    def loadStatFromCsvFiles(self) -> bool:
        ...
        if any(not os.path.exists(os.path.join(self.__work_dir, csv_file)) for csv_file in self.csv_files):
            return False

        self.aa_filt = self.loadStatFromCsvFile(os.path.join(self.__work_dir, 'aa_filt.csv'), CS_STAT_AA_THRESHOLD)
        self.aa_full = self.loadStatFromCsvFile(os.path.join(self.__work_dir, 'aa_full.csv'), CS_STAT_AA_THRESHOLD)

        self.dna_filt = self.loadStatFromCsvFile(os.path.join(self.__work_dir, 'dna_filt.csv'), CS_STAT_NA_THRESHOLD)
        self.dna_full = self.loadStatFromCsvFile(os.path.join(self.__work_dir, 'dna_full.csv'), CS_STAT_NA_THRESHOLD)

        self.rna_filt = self.loadStatFromCsvFile(os.path.join(self.__work_dir, 'rna_filt.csv'), CS_STAT_NA_THRESHOLD)
        self.rna_full = self.loadStatFromCsvFile(os.path.join(self.__work_dir, 'rna_full.csv'), CS_STAT_NA_THRESHOLD)

        if not self.__lazy_others:
            self.others = self.loadStatFromCsvFile(os.path.join(self.__work_dir, 'others.csv'),
    ...
    def loadOtherStatFromCsvFiles(self, comp_id_interest: Optional[str] = None) -> bool:
        ...
        if comp_id_interest is None:
            ...
            stat = self.loadStatFromCsvFile(os.path.join(self.__work_dir, 'others.csv'),
    ...
    def loadStatFromCsvFile(self, file_name: str, primary_th: float, secondary_th: Optional[float] = None,
                            comp_id_interest: Optional[str] = None
                            ) -> List[dict]:
        ...
        with open(file_name, 'r', encoding='utf-8') as ifh:
            ...
            for row in reader:
                ...
                elif comp_id == 'HEB' and (re.match(r'^HM[A-D]1$', _atom_id) is not None or _atom_id == 'HBB1'):
        ...
        if secondary_th is not None:  # extract rest of atoms for non-standard residues
            ...
            for comp_id in comp_ids:
                ...
                if self.__ccU.updateChemCompDict(comp_id):
                    ...
                    for a in self.__ccU.lastAtomDictList:

                        if a['type_symbol'] not in ('H', 'C', 'N', 'P'):
                            continue
                        ...
                        if a['leaving_atom_flag'] != 'Y'\
                           or (peptide_like
                            ...
                            if not any(True for item in atm_list
                                       if item['comp_id'] == comp_id and item['atom_id'] == a['atom_id']):
                                _row = {}
                                _row['comp_id'] = comp_id
                                _row['atom_id'] = a['atom_id']
                                _row['desc'] = 'isolated'
                                _row['primary'] = False
                                _row['norm_freq'] = None
                                _row['count'] = 0
        ...
        with open(file_name, 'r', encoding='utf-8') as ifh:
            ...
            for row in reader:
                ...
                if cc_rel_status == 'OBS' and 'replaced_by' in self.__ccU.lastChemCompDict:
                    replaced_by = self.__ccU.lastChemCompDict['replaced_by']
                    if replaced_by in EMPTY_VALUE or not self.__ccU.updateChemCompDict(replaced_by):
                        continue
                ...
                # geminal proton group
                    ...
                    has_geminal_stat = any(item['desc'] == 'geminal' for item in prev_atm_list
                                           if item['atom_id'] == _atom_id and 'avg' in item)
                    has_aroma_opposit_stat = any(item['desc'].startswith('aroma-opposite') for item in prev_atm_list
                                                 if item['atom_id'] == _atom_id and 'avg' in item)
                    has_methyl_geminal_stat = any(item['desc'] == 'methyl-geminal' for item in prev_atm_list
                                                  if item['atom_id'] == _atom_id and 'avg' in item)
                    ...
                    if len(_atom_id) > 2 and has_geminal_stat:
                        ...
                        if len(others) > 0 and not any('avg' in item for item in prev_atm_list if item['atom_id'] == others[0])\
                           ...
                           and (_atom_id.endswith("'") or not others[0].endswith("'")):
                            ...
                            and any(rep_methylene_proton.startswith(_atom_id[:-1])
                                    for rep_methylene_proton in rep_methylene_protons):
                    ...
                    if len(_atom_id) > 3 and has_methyl_geminal_stat:
                        _item = next(item for item in prev_atm_list
                                     if item['atom_id'] == _atom_id and 'avg' in item and item['desc'] == 'methyl-geminal')
                        others = [item['atom_id'] for item in prev_atm_list
                                  if item['desc'] == _item['desc'] and item['atom_id'].startswith(_item['atom_id'][:-2])
                                  and item['atom_id'] != _item['atom_id']]
    ...
    def __appendExtraFromCcd(self, comp_id: Optional[str]) -> None:
    ...
    def checkAtomNomenclature(self, atom_id: str, comp_id: Optional[str] = None, verbose: bool = False
        ...
        if cc_rel_status == 'OBS' and 'replaced_by' in self.__ccU.lastChemCompDict:
            replaced_by = self.__ccU.lastChemCompDict['replaced_by']
            if replaced_by not in EMPTY_VALUE and self.__ccU.updateChemCompDict(replaced_by):
                if verbose:
                    self.__log.write(f"+{self.__class_name__}.checkAtomNomenclature() "
                                     f"++ Warning  - {comp_id} is replaced by {replaced_by}\n")

                comp_id = replaced_by
                ref_atom_ids = [a['atom_id'] for a in self.__ccU.lastAtomDictList]

                cc_rel_status = self.__ccU.lastChemCompDict['release_status']

        if cc_rel_status == 'REL':
            if any(True for a in self.__ccU.lastAtomDictList
        ...
        _ref_atom_ids = [a for a in ref_atom_ids if a[0] == atom_id[0] or a[0] == 'H' and atom_id[0] in PSE_PRO_BEGIN_CODE]

        if len(_ref_atom_ids) == 0:
            if verbose:
                self.__log.write(f"+{self.__class_name__}.checkAtomNomenclature() "
                                 f"++ Warning  - {comp_id}:{atom_id} did not match with any atom in CCD. No candidates found\n")
    ...
    def __detectMethylProtonFromAtomNomenclature(self, comp_ids: List[str], atm_list: List[dict]) -> None:
        ...
        for comp_id in comp_ids:
            ...
            if self.__ccU.updateChemCompDict(comp_id):
                c_h_bonds = collections.Counter([b['atom_id_1'] for b in self.__ccU.lastBondDictList
                                                 if b['atom_id_1'].startswith('C')
                                                 and b['atom_id_2'][0] in PROTON_BEGIN_CODE])
                ...
                for k, v in c_h_bonds.items():
                    if v == 3:
                        ...
                        for a in h_list:
                            atom_id = a['atom_id']
                            if any(True for b in self.__ccU.lastBondDictList
                                   if b['atom_id_1'] == k and b['atom_id_2'] == atom_id):
                                a['desc'] = 'methyl'
                ...
                for h in h_common:
                    for a in h_list:
                        atom_id = a['atom_id']
                        if atom_id in (f'{h}1', f'{h}2', f'{h}3'):
                            a['desc'] = 'methyl'

    def __detectGeminalProtonFromAtomNomenclature(self, comp_ids: List[str], atm_list: List[dict]) -> None:
            ...
            if self.__ccU.updateChemCompDict(comp_id):
                ...
                for aro in aro_list:
                    for a in h_list:
                        atom_id = a['atom_id']
                        if any(True for b in self.__ccU.lastBondDictList
                               if b['atom_id_1'] == aro and b['atom_id_2'] == atom_id):
                            a['desc'] = 'aroma'

                peptide_like = self.__ccU.peptideLike()

                leaving_atom_list = [a['atom_id'] for a in self.__ccU.lastAtomDictList
                                     if not (a['leaving_atom_flag'] != 'Y'
                                             or (peptide_like
                                                 and a['n_terminal_atom_flag'] == 'N'
                                                 and a['c_terminal_atom_flag'] == 'N'))]

                cn_h_bonds = collections.Counter([b['atom_id_1'] for b in self.__ccU.lastBondDictList
                                                  if b['atom_id_2'][0] in PROTON_BEGIN_CODE
                                                  and b['atom_id_2'] not in leaving_atom_list])
                ...
                h_list = [a for a in _list if a['atom_id'][0] in PROTON_BEGIN_CODE and a['desc'] == 'aroma']
                ...
                for h_1 in h_list:
                    if h_1['atom_id'][-1] in ('1', '2', '3'):
                        hvy_1 = next(b['atom_id_1'] for b in self.__ccU.lastBondDictList
                                     if b['atom_id_2'] == h_1['atom_id'])
                        for h_2 in h_list:
                            if h_2['atom_id'][-1] in ('1', '2', '3') and h_list.index(h_1) < h_list.index(h_2):
                                hvy_2 = next(b['atom_id_1'] for b in self.__ccU.lastBondDictList
                                             if b['atom_id_2'] == h_2['atom_id'])
                                if hvy_1[:-1] == hvy_2[:-1]:
                                    hvy_1_c = set(b['atom_id_1'] for b in self.__ccU.lastBondDictList
                                                  if b['aromatic_flag'] == 'Y' and b['atom_id_2'] == hvy_1) |\
                                        set(b['atom_id_2'] for b in self.__ccU.lastBondDictList
                                            if b['aromatic_flag'] == 'Y' and b['atom_id_1'] == hvy_1)
                                    hvy_2_c = set(b['atom_id_1'] for b in self.__ccU.lastBondDictList
                                                  if b['aromatic_flag'] == 'Y' and b['atom_id_2'] == hvy_2) |\
                                        set(b['atom_id_2'] for b in self.__ccU.lastBondDictList
                                            if b['aromatic_flag'] == 'Y' and b['atom_id_1'] == hvy_2)
                ...
                if pair > 0 and pair % 2 == 0:
                    ...
                    for hvy_c_1 in hvy_c_list:
                        for hvy_c_2 in hvy_c_list:
                            if hvy_c_1 < hvy_c_2:
                                hvy_set_1 = set(b['atom_id_1'] for b in self.__ccU.lastBondDictList
                                                if b['aromatic_flag'] == 'Y' and b['atom_id_2'] == hvy_c_1) |\
                                    set(b['atom_id_2'] for b in self.__ccU.lastBondDictList
                                        if b['aromatic_flag'] == 'Y' and b['atom_id_1'] == hvy_c_1)
                                hvy_set_2 = set(b['atom_id_1'] for b in self.__ccU.lastBondDictList
                                                if b['aromatic_flag'] == 'Y' and b['atom_id_2'] == hvy_c_2) |\
                                    set(b['atom_id_2'] for b in self.__ccU.lastBondDictList
                                        if b['aromatic_flag'] == 'Y' and b['atom_id_1'] == hvy_c_2)
                                ...
                                for hvy_1 in hvy_set_1:
                                    ...
                                    for hvy_2 in hvy_set_2:
                                        if in_ring:
                                            break
                                        if any(True for b in self.__ccU.lastBondDictList
                                               if b['aromatic_flag'] == 'Y'
                                               and b['atom_id_1'] == hvy_1 and b['atom_id_2'] == hvy_2) or\
                                           any(True for b in self.__ccU.lastBondDictList
                                               if b['aromatic_flag'] == 'Y'
                                               and b['atom_id_1'] == hvy_2 and b['atom_id_2'] == hvy_1):
                                            in_ring = True
                                if in_ring:
                ...
                c_1 = [f"H{a['atom_id'][1:-1]}" for a in c_list if a['atom_id'].endswith('1')]
                c_2 = [f"H{a['atom_id'][1:-1]}" for a in c_list if a['atom_id'].endswith('2')]
                c_3 = [f"H{a['atom_id'][1:-1]}" for a in c_list if a['atom_id'].endswith('3')]

                n_list = [a for a in _list if a['atom_id'].startswith('N')]

                n_1 = [f"H{a['atom_id'][1:-1]}" for a in n_list if a['atom_id'].endswith('1')]
                n_2 = [f"H{a['atom_id'][1:-1]}" for a in n_list if a['atom_id'].endswith('2')]
                n_3 = [f"H{a['atom_id'][1:-1]}" for a in n_list if a['atom_id'].endswith('3')]
                ...
                for h in h_common:
                    for a in h_list:
                        atom_id = a['atom_id']
                        if atom_id in (f'{h}1', f'{h}2'):
                            atom_id = f"N{a['atom_id'][1:]}"
                ...
                c_c = [f"H{a['atom_id'][1:-1]}" for a in c_list if a['atom_id'].endswith("'") and not a['atom_id'].endswith("''")]
                c_cc = [f"H{a['atom_id'][1:-2]}" for a in c_list if a['atom_id'].endswith("''")]
    ...
    def __detectGeminalCarbon(self, comp_ids: List[str], atm_list: List[dict]) -> None:
        ...
        for comp_id in comp_ids:
            ...
            if self.__ccU.updateChemCompDict(comp_id):
                ...
                for methyl_c_1 in methyl_c_list:
                    for methyl_c_2 in methyl_c_list:
                        if methyl_c_list.index(methyl_c_1) < methyl_c_list.index(methyl_c_2):
                            hvy_1_c = set(b['atom_id_2'] for b in self.__ccU.lastBondDictList
                                          if b['atom_id_1'] == methyl_c_1
                                          and b['atom_id_2'][0] not in PROTON_BEGIN_CODE) |\
                                set(b['atom_id_1'] for b in self.__ccU.lastBondDictList
                                    if b['atom_id_2'] == methyl_c_1
                                    and b['atom_id_1'][0] not in PROTON_BEGIN_CODE)
                            hvy_2_c = set(b['atom_id_2'] for b in self.__ccU.lastBondDictList
                                          if b['atom_id_1'] == methyl_c_2
                                          and b['atom_id_2'][0] not in PROTON_BEGIN_CODE) |\
                                set(b['atom_id_1'] for b in self.__ccU.lastBondDictList
                                    if b['atom_id_2'] == methyl_c_2
                                    and b['atom_id_1'][0] not in PROTON_BEGIN_CODE)
                            hvy_common = hvy_1_c & hvy_2_c
                            if len(hvy_common) > 0:
                                for hvy_c in hvy_common:
                                    v = len([b for b in self.__ccU.lastBondDictList
                                             if b['atom_id_1'] == hvy_c and b['atom_id_2'] in methyl_c_list]) +\
                                        len([b for b in self.__ccU.lastBondDictList
                                             if b['atom_id_2'] == hvy_c and b['atom_id_1'] in methyl_c_list])

                                    if v in (2, 3):  # '3' accounts for M3L:NZ -> [CM1, CM2, CM3]
                                        for a in _list:
                                            if a['atom_id'] == methyl_c_1 or a['atom_id'] == methyl_c_2:
                                                a['desc'] = 'methyl-geminal'

                                                for methyl_h in [b['atom_id_2'] for b in self.__ccU.lastBondDictList
                                                                 if b['atom_id_1'] == methyl_c_1
                                                                 and b['atom_id_2'][0] in PROTON_BEGIN_CODE]:
                                                    for b in _list:
                ...
                methyl_list = [f"C{a['atom_id'][1:-1]}" for a in _list
                               if a['atom_id'][0] in PROTON_BEGIN_CODE and a['desc'] == 'methyl' and a['atom_id'].endswith('1')]
                ...
                for m in methyl_common:
                    for c in c_list:
                        atom_id = c['atom_id']
                        if atom_id in (f'{m}1', f'{m}2'):
                            c['desc'] = 'methyl-geminal'
                            for h in [a for a in _list
                                      if a['atom_id'][0] in PROTON_BEGIN_CODE and a['desc'] == 'methyl'
                                      and a['atom_id'].startswith(f"H{atom_id[1:]}")]:
                                h['desc'] = 'methyl-geminal'

                aroma_list = [f"C{a['atom_id'][1:]}" for a in _list
                              if a['atom_id'][0] in PROTON_BEGIN_CODE and a['desc'] == 'aroma']

                for c in c_list:
                    if c['atom_id'] in aroma_list:
                        c['desc'] = 'aroma'

                aroma_opposite_list = [f"C{a['atom_id'][1:]}" for a in _list
                                       if a['atom_id'][0] in PROTON_BEGIN_CODE and a['desc'] == 'aroma-opposite']

                for c in c_list:
                    if c['atom_id'] in aroma_opposite_list:
                        c['desc'] = 'aroma-opposite'

    def __detectGeminalNitrogen(self, comp_ids: List[str], atm_list: List[dict]) -> None:
        ...
        for comp_id in comp_ids:
            ...
            geminal_n_list = [f"N{a['atom_id'][1:-1]}" for a in _list
                              if a['atom_id'][0] in PROTON_BEGIN_CODE and a['desc'] == 'geminal' and a['atom_id'].endswith('1')]
                ...
                for geminal_n_1 in geminal_n_list:
                    for geminal_n_2 in geminal_n_list:
                        if geminal_n_list.index(geminal_n_1) < geminal_n_list.index(geminal_n_2):
                            hvy_1_c = set(b['atom_id_2'] for b in self.__ccU.lastBondDictList
                                          if b['atom_id_1'] == geminal_n_1
                                          and b['atom_id_2'][0] not in PROTON_BEGIN_CODE) |\
                                set(b['atom_id_1'] for b in self.__ccU.lastBondDictList
                                    if b['atom_id_2'] == geminal_n_1
                                    and b['atom_id_1'][0] not in PROTON_BEGIN_CODE)
                            hvy_2_c = set(b['atom_id_2'] for b in self.__ccU.lastBondDictList
                                          if b['atom_id_1'] == geminal_n_2
                                          and b['atom_id_2'][0] not in PROTON_BEGIN_CODE) |\
                                set(b['atom_id_1'] for b in self.__ccU.lastBondDictList
                                    if b['atom_id_2'] == geminal_n_2
                                    and b['atom_id_1'][0] not in PROTON_BEGIN_CODE)
                            hvy_common = hvy_1_c & hvy_2_c
                            if len(hvy_common) > 0:
                                for hvy_c in hvy_common:
                                    v = len([b for b in self.__ccU.lastBondDictList
                                             if b['atom_id_1'] == hvy_c
                                             and b['atom_id_2'] in geminal_n_list]) +\
                                        len([b for b in self.__ccU.lastBondDictList
                                             if b['atom_id_2'] == hvy_c
                                             and b['atom_id_1'] in geminal_n_list])
    ...
    def writeStatAsPickleFiles(self) -> None:
        ...
        write_stat_as_pickle(self.dna_filt, os.path.join(self.__work_dir, 'dna_filt.pkl'))
        write_stat_as_pickle(self.dna_full, os.path.join(self.__work_dir, 'dna_full.pkl'))

        write_stat_as_pickle(self.rna_filt, os.path.join(self.__work_dir, 'rna_filt.pkl'))
        write_stat_as_pickle(self.rna_full, os.path.join(self.__work_dir, 'rna_full.pkl'))

        self.loadOtherStatFromCsvFiles()

        write_stat_as_pickle(sorted(self.others, key=itemgetter('comp_id')),
                             os.path.join(self.__work_dir, 'others.pkl'))
    ...
    def loadStatFromPickleFiles(self) -> bool:
        ...
        pickle_files = (os.path.join(self.__work_dir, 'aa_filt.pkl'),
                        os.path.join(self.__work_dir, 'aa_full.pkl'),
                        os.path.join(self.__work_dir, 'dna_filt.pkl'),
                        os.path.join(self.__work_dir, 'dna_full.pkl'),
                        os.path.join(self.__work_dir, 'rna_filt.pkl'),
                        os.path.join(self.__work_dir, 'rna_full.pkl'),
                        os.path.join(self.__work_dir, 'others.pkl'))
        ...
        self.aa_filt = load_stat_from_pickle(os.path.join(self.__work_dir, 'aa_filt.pkl'))
        self.aa_full = load_stat_from_pickle(os.path.join(self.__work_dir, 'aa_full.pkl'))

        self.dna_filt = load_stat_from_pickle(os.path.join(self.__work_dir, 'dna_filt.pkl'))
        self.dna_full = load_stat_from_pickle(os.path.join(self.__work_dir, 'dna_full.pkl'))

        self.rna_filt = load_stat_from_pickle(os.path.join(self.__work_dir, 'rna_filt.pkl'))
        self.rna_full = load_stat_from_pickle(os.path.join(self.__work_dir, 'rna_full.pkl'))

        self.others = load_stat_from_pickle(os.path.join(self.__work_dir, 'others.pkl'))

        self.__updateCompIdSet()

        return True

    def __updateCompIdSet(self) -> None:
    ...
    def updateStatCsvFiles(self) -> bool:
        ...
        for csv_file in self.csv_files:
            ...
            if r.status_code != 200:
                self.__log.write(f"+{self.__class_name__}.updateStatCsvFiles() ++ Warning  - {r}\n")
                return False
            ...
            if not os.path.exists(os.path.join(self.__work_dir, csv_file)):
                ...
                file_last_modified = datetime.datetime.fromtimestamp(
                    os.path.getmtime(os.path.join(self.__work_dir, csv_file))).astimezone()
                if url_last_modified > file_last_modified:
                    update_csv_file(csv_file)
                else:
                    print(f'{os.path.join(self.__work_dir, csv_file)} is update (Last-Modified: {url_last_modified})')
    ...
    def testAtomNomenclatureOfLibrary(self) -> bool:
        ...
        def check_bmrb_cs_stat(atm_list):
            ...
            for comp_id in comp_ids:
                ...
                if len(ref_atom_ids) == 0 or cc_rel_status != 'REL':
                    if comp_id in CS_STAT_IGNORED_LIG_CODE:
    ...
    def getAtomLikeNameSet(self, excl_minor_atom: bool = False, primary: bool = False, minimum_len: int = 1
                           ) -> Set[str]:
        ...
        for comp_id in self.__std_comp_ids:
            ...
            for name in name_list:
                ...
                if name in methyl_list:
                    ...
                    _name = name[:-1]
                    if _name[0] == 'H':
                        name_set.add(f'M{_name[1:]}')
                        if ambig_code == 2:
                            name_set.add(f'QQ{_name[1]}')
                        name_set.add(f'{_name}#')
                        name_set.add(f'{_name}%')
                        name_set.add(f'{_name}*')
                    elif ambig_code == 2:
                        name_set.add(f'{_name}#')
                    ...
                    if geminal_name is not None:
                        ...
                        _name = name[:-1]
                        if _name[0] == 'H':
                            name_set.add(f'Q{_name[1:]}')
                            if geminal_name[:-1].isdigit():
                                name_set.add(f'{_name}#')
                                name_set.add(f'{_name}%')
                                name_set.add(f'{_name}*')
                                name_set.add(f'{_name}X')
                                name_set.add(f'{_name}Y')
