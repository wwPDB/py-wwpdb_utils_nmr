...
class NmrDpUtility(object):
    ...
    def __init__(self, verbose=False, log=sys.stderr):
        ...
        # whether to report warning about author sequence (NMR-STAR)
    ...
    def __validateCSValue__(self, file_list_id, file_name, file_type, content_subtype, sf_data, sf_framecode, lp_category):
        ...
        try:
            ...
            for l, i in enumerate(lp_data):
                ...
                if file_type == 'nef' or (not self.__combined_mode and (atom_id.startswith('Q') or atom_id.startswith('M'))):
                            ...
                            if self.__csStat.hasEnoughStat(comp_id, polypeptide_like):
                                ...
                                if (value < min_value - tolerance or value > max_value + tolerance) and abs(z_score) > 10.0:
                                    ...
                                    if na is None and pa is None:
                                        ...
                                        err = chk_row_tmp % (chain_id, seq_id, comp_id, atom_name) +\
                                              '] %s %s (%s:%s:%s, atom_id %s) is not within expected range (avg %s, std %s, min %s, max %s, Z_score %.2f). Neither aromatic ring nor paramagnetic atom were found in the vicinity. Please check for folded/aliased signals.' %\
                                        ...
                                        warn = chk_row_tmp % (chain_id, seq_id, comp_id, atom_name) +\
                                               '] %s %s (%s:%s:%s, atom_id %s) should be verified (avg %s, std %s, min %s, max %s, Z_score %.2f). The nearest paramagnetic atom (%s:%s:%s:%s) is located at %s angstroms.' %\
                                               (value_name, value, chain_id, seq_id, comp_id, atom_name, avg_value, std_value, min_value, max_value, z_score,
                                    ...
                                    if na is None and pa is None:
                                        ...
                                        warn = chk_row_tmp % (chain_id, seq_id, comp_id, atom_name) +\
                                               '] %s %s (%s:%s:%s:%s) must be verified (avg %s, std %s, min %s, max %s, Z_score %.2f). Neither aromatic ring nor paramagnetic atom were found in the vicinity.' %\
                                               (value_name, value, chain_id, seq_id, comp_id, atom_name, avg_value, std_value, min_value, max_value, z_score)
                                        ...
                                        if pa['distance'] > self.vicinity_paramagnetic:
                                            ...
                                            warn = chk_row_tmp % (chain_id, seq_id, comp_id, atom_name) +\
                                                   '] %s %s (%s:%s:%s:%s) should be verified (avg %s, std %s, min %s, max %s, Z_score %.2f). The nearest paramagnetic atom (%s:%s:%s:%s) is located at %s angstroms.' %\
                                                   (value_name, value, chain_id, seq_id, comp_id, atom_name, avg_value, std_value, min_value, max_value, z_score,
                                                    pa['chain_id'], pa['seq_id'], pa['comp_id'], pa['atom_id'], pa['distance'])
                                    ...
                                    if not na is None:
                                        ...
                                        if pa['distance'] > self.vicinity_paramagnetic:
                                            warn += ' The nearest paramagnetic atom (%s:%s:%s:%s) is located at %s angstroms.' %\
                                                    (pa['chain_id'], pa['seq_id'], pa['comp_id'], pa['atom_id'], pa['distance'])
                                        else:
                                            warn = None
                                ...
                                if min_value < max_value and (value < min_value - tolerance or value > max_value + tolerance) and abs(z_score) > 10.0:
                                    ...
                                    if na is None and pa is None:
                                        ...
                                        err = chk_row_tmp % (chain_id, seq_id, comp_id, atom_name) +\
                                              '] %s %s (%s:%s:%s:%s) is not within expected range (avg %s, std %s, min %s, max %s, Z_score %.2f). Neither aromatic ring nor paramagnetic atom were found in the vicinity. Please check for folded/aliased signals.' %\
                                              (value_name, value, chain_id, seq_id, comp_id, atom_name, avg_value, std_value, min_value, max_value, z_score)
                        ...
                        for i in range(len(s['seq_id'])):
                            ...
                            if his['tautomeric_state_pred'] != 'unknown':
                                ...
                                if not item is None:
                                    ...
                                    warn = "Tautomeric state %s (%s:%s:%s) can not be verified with the assigned chemical shift values (%stautomeric_state_pred %s)." %\
        ...
        try:
            ...
            for l, i in enumerate(lp_data):
                ...
                if target_value is None:
                    ...
                    if lower_limit_name in i and (not i[lower_limit_name] is None) and upper_limit_name in i and (not i[upper_limit_name] is None):
                        target_value = (i[lower_limit_name] + i[upper_limit_name]) / 2.0

                    elif lower_linear_limit_name in i and (not i[lower_linear_limit_name] is None) and upper_linear_limit_name is i and (not i[upper_linear_limit_name] is None):
                        target_value = (i[lower_linear_limit_name] + i[upper_linear_limit_name]) / 2.0

                    elif upper_linear_limit_name in i and (not i[upper_linear_limit_name] is None):
                        target_value = i[upper_linear_limit_name]
                        upper_limit_value = target_value

                    elif upper_limit_name in i and (not i[upper_limit_name] is None):
                        target_value = i[upper_limit_name]
                        upper_limit_value = target_value

                    elif lower_linear_limit_name in i and (not i[lower_linear_limit_name] is None):
                        target_value = i[lower_linear_limit_name]
                        lower_limit_value = target_value

                    elif lower_limit_name in i and (not i[lower_limit_name] is None):
                ...
                if 'hydrogen_bonds' in data_type and 'too close!' in data_type:
                    ...
                    if lower_limit_name in i and not i[lower_limit_name] is None:
                        values += '%s %s, ' % (lower_limit_name, i[lower_limit_name])
                    if upper_limit_name in i and not i[upper_limit_name] is None:
                        values += '%s %s, ' % (upper_limit_name, i[upper_limit_name])
                    if lower_linear_limit_name in i and not i[lower_linear_limit_name] is None:
    ...
    def __getTypeOfDistanceRestraint(self, file_type, lp_data, row_id, target_value, upper_limit_value, lower_limit_value,
        ...
        if hydrogen_bond:
            ...
            if file_type == 'nef' or (not self.__combined_mode and (atom_id_1.startswith('Q') or atom_id_1.startswith('M') or atom_id_2.startswith('Q') or atom_id_2.startswith('M'))):
                ...
                if file_type == 'nef':
                    ...
                    if not atom_id_1.startswith('Q') and not atom_id_1.startswith('M'):
                    ...
                    if not atom_id_2.startswith('Q') and not atom_id_2.startswith('M'):
        ...
        try:
            ...
            for i in lp_data:
                ...
                if target_value is None:
                    ...
                    elif lower_linear_limit_name in i and (not i[lower_linear_limit_name] is None) and upper_linear_limit_name in i and (not i[upper_linear_limit_name] is None):
                        target_value = (i[lower_linear_limit_name] + i[upper_linear_limit_name]) / 2.0
        ...
        for fileListId in range(self.__file_path_list_len):
            ...
            for bond in bond_list:
                ...
                if disulf['redox_state_pred_1'] != 'oxidized' and disulf['redox_state_pred_1'] != 'unknown':
                    ...
                    warn = "Disulfide bond (%s:%s:%s, %s:%s:%s) can not be verified with the assigned chemical shift values (CA_1 %s, CB_1 %s, redox_state_pred_1 %s)." %\
                ...
                if disulf['redox_state_pred_2'] != 'oxidized' and disulf['redox_state_pred_2'] != 'unknown':
                    ...
                    warn = "Disulfide bond (%s:%s:%s, %s:%s:%s) can not be verified with the assigned chemical shift values (CA_2 %s, CB_2 %s, redox_state_pred_2 %s)." %\
        ...
        for fileListId in range(self.__file_path_list_len):
            ...
            for bond in bond_list:
                ...
                if other['redox_state_pred_1'] != 'oxidized' and other['redox_state_pred_1'] != 'unknown':
                    ...
                    warn = "Other bond (%s:%s:%s, %s:%s:%s) can not be verified with the assigned chemical shift values (CA_1 %s, CB_1 %s, redox_state_pred_1 %s)." %\
                ...
                if other['redox_state_pred_2'] != 'oxidized' and other['redox_state_pred_2'] != 'unknown':
                    ...
                    warn = "Other bond (%s:%s:%s, %s:%s:%s) can not be verified with the assigned chemical shift values (CA_2 %s, CB_2 %s, redox_state_pred_2 %s)." %\
    ...
    def __testRestraintPotentialSWP(self, content_subtype, lp_data):
        ...
        try:
            ...
            for i in lp_data:
                if (lower_limit_name in i and not i[lower_limit_name] is None) and\
                   (upper_limit_name in i and not i[upper_limit_name] is None) and\
                   ((not lower_linear_limit_name in i) or i[lower_linear_limit_name] is None) and\
                   ((not upper_linear_limit_name in i) or i[upper_linear_limit_name] is None):
                    continue
    ...
    def __testRestraintPotentialSWPL(self, content_subtype, lp_data):
        ...
        try:
            ...
            for i in lp_data:
                if (lower_limit_name in i and not i[lower_limit_name] is None) and\
                   (upper_limit_name in i and not i[upper_limit_name] is None) and\
                   (lower_linear_limit_name in i and not i[lower_linear_limit_name] is None) and\
                   (upper_linear_limit_name in i and not i[upper_linear_limit_name] is None):
                    continue
    ...
    def __testRestraintPotentialUBP(self, content_subtype, lp_data):
        ...
        try:
            ...
            for i in lp_data:
                if ((not lower_limit_name in i) or i[lower_limit_name] is None) and\
                   (upper_limit_name in i and not i[upper_limit_name] is None) and\
                   ((not lower_linear_limit_name in i) or i[lower_linear_limit_name] is None) and\
                   ((not upper_linear_limit_name in i) or i[upper_linear_limit_name] is None):
                    continue
    ...
    def __testRestraintPotentialLBP(self, content_subtype, lp_data):
        ...
        try:
            ...
            for i in lp_data:
                if (lower_limit_name in i and not i[lower_limit_name] is None) and\
                   ((not upper_limit_name in i) or i[upper_limit_name] is None) and\
                   ((not lower_linear_limit_name in i) or i[lower_linear_limit_name] is None) and\
                   ((not upper_linear_limit_name in i) or i[upper_linear_limit_name] is None):
                    continue
    ...
    def __testRestraintPonentialLHorP(self, content_subtype, lp_data):
        ...
        try:
            ...
            for i in lp_data:
                if (target_value_name in i and not i[target_value_name] is None) and\
                   ((not lower_limit_name in i) or i[lower_limit_name] is None) and\
                   ((not upper_limit_name in i) or i[upper_limit_name] is None) and\
                   ((not lower_linear_limit_name in i) or i[lower_linear_limit_name] is None) and\
                   ((not upper_linear_limit_name in i) or i[upper_linear_limit_name] is None):
                    continue
