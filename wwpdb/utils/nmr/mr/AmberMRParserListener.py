...
class AmberMRParserListener(ParseTreeListener):
    ...
    # whether use alternative polymer sequence
    __useDefault = True
                                ...
                                if iat in self.__sanderAtomNumberDict:
                                    ...
                                    if ge is not None:
                                        ...
                                        if not self.updateSanderAtomNumberDict(factor, useDefault=self.__useDefault):
                                            _factor = self.getAtomNumberDictFromAmbmaskInfo(int(ge[_offset]), ge[_offset + 2].strip(), enableWarning=False)
                                            if _factor is None:
                                                self.__f.append(f"[Invalid data] {self.__getCurrentRestraint()}"
                                                ...
                                                continue
                                            __factor = {'auth_seq_id': int(ge[_offset]),
                                            ...
                                            for atom_id, iat in zip(sorted(atom_ids), igr):
                                                ...
                                                if not self.updateSanderAtomNumberDict(factor, useDefault=self.__useDefault):
                                                    for order, iat in enumerate(igr):
                                                        _factor = self.getAtomNumberDictFromAmbmaskInfo(int(ge[_offset]), atom_id, order, enableWarning=False)
                                                        if _factor is None:
                                                            self.__f.append(f"[Invalid data] {self.__getCurrentRestraint()}"
                                                                            f"Couldn't specify 'igr({varNum})={igr}' in the coordinates "
                                                                            f"based on Sander comment {' '.join(ge[_offset:offset+3]).strip()!r}.")
                                                            break
                                                        _factor = {'auth_seq_id': int(ge[_offset]),
                                                                   'auth_comp_id': _factor['comp_id'],  # pylint: disable=unsubscriptable-object
                                                                   'auth_atom_id': _factor['atom_id'],  # pylint: disable=unsubscriptable-object
                                                                   'iat': iat
                                                                   }
                            ...
                            for col, iat in enumerate(self.iat):
                                ...
                                if iat > 0:
                                    if iat in self.__sanderAtomNumberDict:
                                        ...
                                        _factor = self.getAtomNumberDictFromAmbmaskInfo(seqId, atomId, enableWarning=False, useDefault=False)
                                        if _factor is None:
                                            self.__f.append(f"[Invalid data] {self.__getCurrentRestraint()}"
                                                            f"Couldn't specify 'iat({col+1})={iat}' in the coordinates "
                                        ...
                                        if len(chainIds) != 1 and (self.__reasons is None or 'chain_seq_id_remap' not in self.__reasons):
                                            self.__f.append(f"[Invalid data] {self.__getCurrentRestraint()}"
                                                            f"Couldn't specify 'iat({col+1})={iat}' in the coordinates "
                                                            f"based on Sander comment {self.prevComment!r}.")
                                        elif not self.updateSanderAtomNumberDict(factor, useDefault=False):
                                            self.__f.append(f"[Invalid data] {self.__getCurrentRestraint()}"
                                                            f"Couldn't specify 'iat({col+1})={iat}' in the coordinates "
                                                            f"based on Sander comment {self.prevComment!r}.")
                                ...
                                if iat > 0:
                                    if iat in self.__sanderAtomNumberDict:
                                        ...
                                        if _factor is None:
                                            ...
                                            polySeq = self.__polySeq if self.__altPolySeq is None else self.__altPolySeq
                            ...
                            for col, iat in enumerate(self.iat):
                                ...
                                if iat > 0:
                                    if iat in self.__sanderAtomNumberDict:
                                        ...
                                        if not self.updateSanderAtomNumberDict(factor, useDefault=False):
                                            self.__f.append(f"[Invalid data] {self.__getCurrentRestraint()}"
                                                            f"Couldn't specify 'iat({col+1})={iat}' in the coordinates "
        ...
        for ps in (self.__polySeq if useDefault or self.__altPolySeq is None else self.__altPolySeq):
            ...
            if seqId in (ps['seq_id'] if useDefault and not enforceAuthSeq else ps['auth_seq_id']):
                ...
                if compId not in monDict3 and self.__mrAtomNameMapping is not None:
                    ...
                    _, _, atomId = retrieveAtomIdentFromMRMap(self.__mrAtomNameMapping, seqId, origCompId, authAtomId, compId, coordAtomSite)
                ...
                for np in self.__nonPolySeq:
                    ...
                    if seqId in np['auth_seq_id']:
                        ...
                        if compId not in monDict3 and self.__mrAtomNameMapping is not None:
                            ...
                            _, _, authAtomId = retrieveAtomIdentFromMRMap(self.__mrAtomNameMapping, seqId, origCompId, authAtomId, compId, coordAtomSite)
