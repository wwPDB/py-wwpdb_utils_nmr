...
class AmberPTParserListener(ParseTreeListener):
    ...
    def exitResidue_label_statement(self, ctx: AmberPTParser.Residue_label_statementContext):
        ...
        ps = {'chain_id': self.__polySeq[0]['chain_id']}

        compIdList = []
        i = 0
        while ctx.Simple_name(i):
            chunk = chunk_string(str(ctx.Simple_name(i)).upper(), 4)
            compIdList.extend(chunk)
            i += 1

        ps['comp_id'] = compIdList
        ps['seq_id'] = list(range(len(compIdList)))

        self.__polySeqInMR.append(ps)
    ...
    def enterFormat_function(self, ctx: AmberPTParser.Format_functionContext):  # pylint: disable=unused-argument
        pass
