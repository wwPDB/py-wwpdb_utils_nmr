...
class BareMRParserListener(ParseTreeListener):
        ...
        if target_value is not None:
            ...
            if None not in (lower_limit, upper_limit):
                if lower_limit > upper_limit:
                    validRange = False
                    self.__f.append(f"[Range value error] {self.__getCurrentRestraint()}"
                                    f"The lower limit value='{lower_limit:.3f}' must be less than the upper limit value '{upper_limit}'.")
    ...
    def getPolymerSequence(self) -> Optional[List[dict]]:
        """ Return polymer sequence of Bare WSV/TSV/CSV MR file.
        """
    ...
    def getSequenceAlignment(self) -> Optional[List[dict]]:
        """ Return sequence alignment between coordinates and Bare WSV/TSV/CSV MR.
        """
    ...
    def getChainAssignment(self) -> Optional[List[dict]]:
        """ Return chain assignment between coordinates and Bare WSV/TSV/CSV MR.
        """

        return None if self.__chainAssign is None or len(self.__chainAssign) == 0 else self.__chainAssign

    def getReasonsForReparsing(self) -> Optional[dict]:
        """ Return reasons for re-parsing Bare WSV/TSV/CSV MR file.
        """
