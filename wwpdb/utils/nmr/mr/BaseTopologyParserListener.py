...
class BaseTopologyParserListener():
    ...
    hasPolySeqModel = False
    hasNonPolyModel = False
    ...
    # atoms (CHARMM/GROMACS specific)
    atoms = None
    # previous atom number (CHARMM/GROMACS specific)
    prev_nr = -1
    # current atom number (PDB specific)
    cur_nr = -1
    ...
    def __init__(self, verbose: bool = True, log: IO = sys.stdout,
        ...
        # NEFTranslator
        self.__nefT = NEFTranslator(verbose, log, self.ccU, self.csStat) if nefT is None else nefT
        if nefT is None:
            self.__nefT.set_remediation_mode(True)

        # Pairwise align
        if self.hasPolySeqModel:
