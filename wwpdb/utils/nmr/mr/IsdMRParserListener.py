...
class IsdMRParserListener(ParseTreeListener):
                            ...
                            if chainAssignFailed is not None:
                                ...
                                if len(seqIdRemapFailed) > 0:
                                    if 'chain_seq_id_remap' not in self.reasonsForReParsing:
                                        seqIdRemap = self.reasonsForReParsing['seq_id_remap'] if 'seq_id_remap' in self.reasonsForReParsing else None
                                        if seqIdRemap is None\
                                           or len(seqIdRemap) != len(seqIdRemapFailed)\
    ...
    def selectRealisticBondConstraint(self, atom1, atom2, alt_atom_id1, alt_atom_id2, dst_func):
        ...
        try:
            ...
            if self.__verbose:
                self.__lfh.write(f"+IsdMRParserListener.selectRealisticBondConstraint() ++ Error  - {str(e)}")
