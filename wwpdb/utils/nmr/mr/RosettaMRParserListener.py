...
class RosettaMRParserListener(ParseTreeListener):
    ...
    # min_seq_coverage_w_conflict = 0.9
    ...
    def assignCoordPolymerSequence(self, seqId, atomId=None, fixedChainId=None):
        ...
        for ps in self.__polySeq:
            ...
            if seqId in ps['auth_seq_id']:
                ...
                self.updatePolySeqRst(chainId, _seqId, cifCompId)
                if atomId is None\
        ...
        for row, column in indices:
            ...
            if mat[row][column] >= 0:
                ...
                if len(_cif_chains) > 1:
                    ...
                    self.warningMessage += f"[Concatenated sequence] The chain ID {chain_id2!r} of the sequences in the ROSETTA restraint file "\
                    ...
                    if mr_comp_id == '.' and cif_comp_id != '.':
                        ...
                        if not aligned[i]:
                            ...
                            self.warningMessage += f"[Sequence mismatch] {cif_seq_code} is not present "\
                                f"in the ROSETTA restraint data (chain_id {chain_id2}).\n"
                        """
                    elif mr_comp_id != cif_comp_id and aligned[i]:
                        ...
                        self.warningMessage += f"[Sequence mismatch] Sequence alignment error between the coordinate ({cif_seq_code}) "\
                            f"and the ROSETTA restraint data ({mr_seq_code}). "\
