            ...
            for chainId in _factor['chain_id']:
                ...
                for seqId in _factor['seq_id']:
                    ...
                    if not cur_subtype_w_axis and not cur_sybtype_w_para:
                        self.updatePolySeqRst(chainId, seqId, compId)
        ...
        for row, column in indices:
            ...
            if mat[row][column] >= 0:
                ...
                if len(_cif_chains) > 1:
                    ...
                    self.warningMessage += f"[Concatenated sequence] The chain ID {chain_id2!r} of the sequences in the XPLOR-NIH restraint file "\
                ...
                for i in range(length):
                    ...
                    if mr_comp_id == '.' and cif_comp_id != '.':
                        ...
                        self.warningMessage += f"[Sequence mismatch] Sequence alignment error between the coordinate ({cif_seq_code}) "\
                            f"and the XPLOR-NIH restraint data ({mr_seq_code}). "\
