...
# Update:
##
""" A collection of classes for parsing ARIA PK files.
"""
import sys
import os
...
try:
    ...
    from wwpdb.utils.nmr.pk.AriaPKParserListener import AriaPKParserListener
    ...
    from nmr.pk.AriaPKParserListener import AriaPKParserListener
...
class AriaPKReader:
    """ Accessor methods for parsing ARIA PK files.
    """
        ...
        try:
            ...
            if isFilePath:
                ...
                if not os.access(pkFilePath, os.R_OK):
                    if self.__verbose:
                        self.__lfh.write(f"AriaPKReader.parse() {pkFilePath} is not accessible.\n")
                ...
                if pkString is None or len(pkString) == 0:
                    if self.__verbose:
                        self.__lfh.write("AriaPKReader.parse() Empty string.\n")
                    return None, None, None
            ...
            if cifFilePath is not None:
                if not os.access(cifFilePath, os.R_OK):
                    if self.__verbose:
                        self.__lfh.write(f"AriaPKReader.parse() {cifFilePath} is not accessible.\n")
                    return None, None, None
            ...
            listener = AriaPKParserListener(self.__verbose, self.__lfh,
            ...
            if self.__verbose:
                self.__lfh.write(f"+AriaPKReader.parse() ++ Error - {str(e)}\n")
            return None, None, None
            # pylint: disable=unreachable
            """ debug code
        except Exception as e:
            if self.__verbose and isFilePath:
                self.__lfh.write(f"+AriaPKReader.parse() ++ Error - {pkFilePath!r} - {str(e)}\n")
            return None, None, None
            """
        finally:
